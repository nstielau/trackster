class WelcomeController < ApplicationController
  def index
    ...
    @user_data = TrackAggregator.new({:user_id => current_user ? current_user.id : nil, :interval => 'day', :start_time => 1.month.ago}).result
  end
  ...
  def twitter_dashboard
    @twitter_data = TwitterTrackAggregator.new({:interval => 'day', :start_time => 60.days.ago}).result
  end
end